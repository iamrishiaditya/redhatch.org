let editMode = false // Set to false so the graphics take up the whole screen
let resolution = 0.5 
let renderDelay = 1000 
let dpr = Math.max(1, resolution * window.devicePixelRatio)
let frm, source, editor, store, renderer, pointers
const shaderId = 'PLpQzjW'
window.onload = init
...
class Renderer {
  ...
  createCubeMap() {
    ...
    const faces = [[gl.TEXTURE_CUBE_MAP_POSITIVE_X, '01posx.jpg'],[gl.TEXTURE_CUBE_MAP_NEGATIVE_X, '01negx.jpg'],[gl.TEXTURE_CUBE_MAP_POSITIVE_Y, '01posy.jpg'],[gl.TEXTURE_CUBE_MAP_NEGATIVE_Y, '01negy.jpg'],[gl.TEXTURE_CUBE_MAP_POSITIVE_Z, '01posz.jpg'],[gl.TEXTURE_CUBE_MAP_NEGATIVE_Z, '01negz.jpg']
    ]
...
class Store {
  ...
  putShaderSource(name, source) { this.store.setItem(this.#getKey(this.#StorageType.shader, name), source) }
  ...
  deleteOwnShader(uuid) {
    const ownShaders = this.getOwnShaders()
    this.store.setItem(this.#getKey(this.#StorageType.config, this.#ownShadersKey), JSON.stringify(ownShaders.filter((s) => s.uuid !== uuid)))
...
class PointerHandler {
  constructor(element, scale) {
    this.scale = scale; this.active = false; this.pointers = new Map(); this.lastCoords = [0,0]; this.moves = [0,0]
    const map = (element, scale, x, y) => [x * scale, element.height - y * scale]
    element.addEventListener("pointerdown", (e) => { this.active = true; this.pointers.set(e.pointerId, map(element, this.getScale(), e.clientX, e.clientY)) })
    ...
    element.addEventListener("pointermove", (e) => { if (!this.active) return; this.lastCoords =[e.clientX, e.clientY]; this.pointers.set(e.pointerId, map(element, this.getScale(), e.clientX, e.clientY)); this.moves = [this.moves[0]+e.movementX, this.moves[1]+e.movementY] })
  }
...
class Editor {
  constructor(textarea, errorfield, errorindicator) {
    this.textarea = textarea; this.errorfield = errorfield; this.errorindicator = errorindicator
    textarea.addEventListener('keydown', this.handleKeydown.bind(this)); textarea.addEventListener('scroll', this.handleScroll.bind(this))
  }
  ...
  setError(message) {
    this.errorfield.innerHTML = message; this.errorfield.classList.add('opaque')
    const match = message.match(/ERROR: \d+:(\d+):/); const lineNumber = match ? parseInt(match[1]) : 0
    const overlay = document.createElement('pre'); overlay.classList.add('overlay'); overlay.textContent = '\n'.repeat(lineNumber)
    document.body.appendChild(overlay)
    const offsetTop = parseInt(getComputedStyle(overlay).height)
    this.errorindicator.style.setProperty('--top', offsetTop + 'px'); this.errorindicator.style.visibility = 'visible'
    document.body.removeChild(overlay)
  }
  clearError() { this.errorfield.textContent = ''; this.errorfield.classList.remove('opaque'); this.errorindicator.style.visibility = 'hidden' }
  focus() { this.textarea.focus() }
  #hide() {[this.errorindicator, this.errorfield, this.textarea].forEach(el => el.classList.add('hidden')) }
  #show() { [this.errorindicator, this.errorfield, this.textarea].forEach(el => el.classList.remove('hidden')); this.focus() }
  handleScroll() { this.errorindicator.style.setProperty('--scroll-top', `${this.textarea.scrollTop}px`) }
  handleKeydown(event) { if (event.key === "Tab") { event.preventDefault(); this.handleTabKey(event.shiftKey) } else if (event.key === "Enter") { event.preventDefault(); this.handleEnterKey() } }
  handleTabKey(shiftPressed) { if (this.#getSelectedText() !== "") { shiftPressed ? this.#unindentSelectedText() : this.#indentSelectedText() } else { this.#indentAtCursor() } }
  #getSelectedText() { return this.textarea.value.substring(this.textarea.selectionStart, this.textarea.selectionEnd) }
  #indentAtCursor() { document.execCommand('insertText', false, '\t'); this.textarea.selectionStart = this.textarea.selectionEnd = this.textarea.selectionStart + 1 }
  #indentSelectedText() { const cursorPos = this.textarea.selectionStart; const indentedText = this.#getSelectedText().split('\n').map(line => '\t' + line).join('\n'); document.execCommand('insertText', false, indentedText); this.textarea.selectionStart = cursorPos }
  #unindentSelectedText() { const cursorPos = this.textarea.selectionStart; const indentedText = this.#getSelectedText().split('\n').map(line => line.replace(/^\t/, '').replace(/^ /, '')).join('\n'); document.execCommand('insertText', false, indentedText); this.textarea.selectionStart = cursorPos }
  handleEnterKey() {
    const editor = this.textarea; const visibleTop = editor.scrollTop; const cursorPosition = editor.selectionStart; let start = cursorPosition - 1
    while (start >= 0 && editor.value[start] !== '\n') start--; let newLine = ''
    while (start < cursorPosition - 1 && (editor.value[start + 1] === ' ' || editor.value[start + 1] === '\t')) { newLine += editor.value[start + 1]; start++ }
    document.execCommand('insertText', false, '\n' + newLine)
    editor.selectionStart = editor.selectionEnd = cursorPosition + 1 + newLine.length; editor.scrollTop = visibleTop
  }
}